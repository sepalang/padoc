    ...
    if(interactive === false){
      ...
      babel.stdout.on('data',(data)=>{
        ...
        if(interactive == true && data.toString().indexOf(">") > -1){
          ...
          const realRunScript = Buffer.from(
            `const self=require('${realInputPath}'); const $$=()=>\`The following attributes are set to global. [\$\{ Object.keys(self).map(key=>{global[key]=self[key]; return key; }) \}]\`; console.log("global.self << ",self); global.self = self;  'Loading of the module is done. To use exports, check the "self" variable. or $$()';\n`, 'utf8'
          )
          ...
          babel.stdin.write(realRunScript)
        }
